...
import { clone, concat, forEach, indexOf, last } from 'lodash'
...
// const map = Function.prototype.call.bind([].map)

const DIFF_MODE_NORMAL = 0
const DIFF_MODE_INSERT = 1
const DIFF_MODE_DELETE = 2

// Space between list elements
const LI_MARGIN_BOTTOM = 8
...
const lineNumberMode = ''

// function convertHTML(html, lineNumberMode) {
...
function extractLineNumbers(element) {
...
function parseChildren(converted, element, currentParagraph, styles, diff_mode) {
  var elements = []
  var children = element.childNodes
...
function isInsideAList(element) {
...
function ParseElement(alreadyConverted, element, currentParagraph, styles, diff_mode) {
...
function ParseHtml(converted, htmlText) {
  var html = validize(htmlText)
...
function getLineNumberObject(line) {
 ...
 function validize(str) {
  if (str) {
    str = replaceMalformedUmlauts(str)
    ...
    a.innerHTML = str
    a.childNodes.forEach(function (child) {
      if (child.nodeType == 1) {
        return str
      }
    })
...
function replaceMalformedUmlauts(text) {
